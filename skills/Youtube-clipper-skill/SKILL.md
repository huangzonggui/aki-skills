...
> **虚拟环境**：本 Skill 使用 `scripts/py` 自动创建/使用本地 `.venv` 并安装 `requirements.txt`，无需手动激活虚拟环境。
...
3. 检测 Python 依赖
   ```bash
   scripts/py -c "import yt_dlp; print('✅ yt-dlp available')"
   scripts/py -c "import pysrt; print('✅ pysrt available')"
   ```
...
2. 调用 download_video.py 脚本
   ...
   scripts/py scripts/download_video.py <youtube_url>
   ```
...
1. 调用 analyze_subtitles.py 解析 VTT 字幕
   ```bash
   scripts/py scripts/analyze_subtitles.py <subtitle_path>
   ```
...
#### 5.1 剪辑视频片段
```bash
scripts/py scripts/clip_video.py <video_path> <start_time> <end_time> <output_path>
```
- 使用 FFmpeg 精确剪辑
- 保持原始视频质量
- 输出: `<章节标题>_clip.mp4`
...
#### 5.3 翻译字幕（如果用户选择）
```bash
scripts/py scripts/translate_subtitles.py <subtitle_path>
```
...
#### 5.5 烧录字幕到视频（如果用户选择）
```bash
scripts/py scripts/burn_subtitles.py <video_path> <subtitle_path> <output_path>
```
...
#### 5.6 生成总结文案（如果用户选择）
```bash
scripts/py scripts/generate_summary.py <chapter_info>
```
- 基于章节标题、摘要和关键词
- 生成适合社交媒体的文案
