...
It uses Comfly Chat Completions (Gemini 3 Pro Preview Thinking by default) and now supports multiple style presets.
...
Use the new tutorial-style preset:

```bash
npx -y bun scripts/generate-html.ts /path/to/article.md --style art-guide
```
...
| `--style <name>` | Style preset: `classic` or `art-guide` (`part-guide`/`wechat-guide` still work) | `art-guide` |
...
# Tutorial / install guide style
npx -y bun scripts/generate-html.ts article.md --style art-guide
...
| Style | Use Case | Visual Direction |
|------|----------|------------------|
| `art-guide` | Tutorials, installation guides, product walkthroughs | Current default layout. Large `PART` section headers, green accents, terminal-style command blocks, muted editorial cards |
| `classic` | General long-form articles | Older general article layout |
...
│   ├── template.html        # Default classic template
│   └── template-part-guide.html
└── references/
    └── usage.md             # This file
```
...
- **aki-post-to-wechat**: Post to WeChat after export
