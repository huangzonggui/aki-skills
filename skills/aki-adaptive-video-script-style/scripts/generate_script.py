...
def _build_prompts(
    ...
    user_prompt = (
        ...
        "硬性要求：\n"
        "1) 前 3-5 秒必须先给抓手。第一句优先从这几类切入：新发布、价格变化、使用门槛、翻车风险、数字清单、反差结论。\n"
        "2) 开头禁止背景铺垫。不要写“今天聊聊”“先说结论”“最近很多人都在问”这类口头垫词。\n"
        "3) 如果目标时长在 35 秒以内，默认只讲 2-3 个重点，不要硬铺满，不要写成长口播。\n"
        "4) 结构不要固定三点。可以是 2 点、3 点，必要时 4 点，但每一点都要短。\n"
        "5) 允许“详细说说：”“一. 二. 三.” 这种口语提纲感，但整体必须仍然是可直接念的成稿，不是文章摘要，也不是会议提纲。\n"
        "6) 短句推进，每行一个信息点，避免长段废话。专业词优先翻成人话。\n"
        "7) 优先用普通人能听懂的话。能不用抽象术语，就不要用抽象术语。\n"
        "8) 少用英文名、缩写词、专业词做句子的核心信息点。除非是品牌名本身就是重点，否则优先翻成人话。\n"
        "9) 禁止空泛升级词：不要只写“更稳了”“都能顶住”“全面升级”“更丝滑”。如果写提升，必须说清楚提升在哪里。\n"
        "10) 要更像人在对镜头讲，不要把原文标题、小标题、列表原样搬过来。\n"
        "11) 禁止模板腔、禁用空泛 CTA。\n"
        "12) 只输出纯口播正文多行，不要标题，不要时长提示，不要任何说明性前缀。\n"
        "13) 不要 markdown 痕迹，不要 `#`、`##`、`---`、列表符号、编号符号。\n\n"
        "待改写内容如下：\n\n"
        f"{content}\n"
    )
