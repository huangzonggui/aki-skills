...
def main() -> None:
    ...
    shared_config_path = DEFAULT_SHARED_CONFIG_PATH.expanduser()
    provider = parse_env_like_file(shared_config_path)
    ...
    settings["api_key"] = str(provider.get("COMFLY_API_KEY") or settings.get("api_key", "")).strip()
    settings["base_url"] = normalize_base_url(
        str(provider.get("COMFLY_API_BASE_URL") or provider.get("COMFLY_API_URL") or settings.get("base_url", "")).strip()
    )
    settings["image_model"] = str(provider.get("COMFLY_IMAGE_MODEL") or settings.get("image_model", "")).strip()
    settings["path"] = str(provider.get("COMFLY_IMAGE_PATH") or settings.get("path", "/v1/images/generations")).strip()
    if args.size:
    ...
    for block in prompt_blocks:
        ...
        for idx, image in enumerate(images, start=1):
            ...
            if image["kind"] == "b64":
                data = decode_base64_image(image["data"])
