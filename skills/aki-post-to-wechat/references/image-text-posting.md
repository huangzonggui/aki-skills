...
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --markdown source.md --images ./images/

# Post with explicit title and content
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --title "标题" --content "内容" --image img1.png --image img2.png

# Save as draft
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --markdown source.md --images ./images/ --submit

# Save as draft + apply pre-submit settings (optional)
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --markdown source.md --images ./images/ --submit --pre-submit-settings

# Save as draft and skip extra settings (optional troubleshooting)
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --markdown source.md --images ./images/ --submit --skip-pre-submit-settings

# Save as draft (default fixed profile: zimeiti-publisher)
npx -y bun ./scripts/wechat-browser.ts --intent imagepost --markdown source.md --images ./images/ --submit
```
...
| `--pre-submit-settings` | Apply 原创/创作来源/赞赏/广告/合集 settings before save (default on) |
| `--skip-pre-submit-settings` | Request skip extra settings (ignored unless `WECHAT_ALLOW_SKIP_PRE_SUBMIT=1`) |
| `--profile <dir>` | Chrome profile directory |
| `--profile-name <name>` | Publisher profile name（默认 `zimeiti-publisher`） |
