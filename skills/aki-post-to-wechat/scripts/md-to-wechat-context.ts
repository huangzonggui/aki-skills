...
async function resolveImagePath(imagePath: string, baseDir: string, tempDir: string): Promise<string> {
  ...
  const directPath = path.resolve(baseDir, imagePath);
  if (fs.existsSync(directPath)) return directPath;

  const parentFallback = path.resolve(baseDir, '..', imagePath);
  if (fs.existsSync(parentFallback)) return parentFallback;
...
function stripSimpleTag(html: string): string {
  return decodeHtmlEntities(html.replace(/^<p[^>]*>/i, '').replace(/<\/p>$/i, '').trim());
}
...
function markdownTableAlign(cell: string): 'left' | 'center' | 'right' {
...
function renderMarkdownTable(lines: string[]): string | null {
  ...
  const rows = lines.slice(2).map(splitMarkdownTableRow).filter((cells) => cells.length > 0);
  const normalize = (cells: string[]): string[] => {
    const copy = cells.slice(0, header.length);
  ...
  const headerCells = normalize(header)
    .map(
      (cell, idx) =>
        `<th style="border: 1px solid #d0d7de; background: #f6f8fa; padding: 10px 8px; font-size: 15px; line-height: 1.5; text-align: ${aligns[idx]}; font-weight: 700; color: #1f2328;">${escapeHtml(cell)}</th>`,
    )
    .join('');
  const bodyRows = rows
    .map((cells) => {
      ...
      const cols = normalized
        .map(
          (cell, idx) =>
            `<td style="border: 1px solid #d0d7de; padding: 10px 8px; font-size: 15px; line-height: 1.6; text-align: ${aligns[idx]}; color: #333;">${escapeHtml(cell)}</td>`,
        )
        .join('');
      return `<tr>${cols}</tr>`;
    })
...
function transformMarkdownTables(contentHtml: string): string {
  const rowPattern = /<p\b[^>]*>\s*\|[\s\S]*?\|\s*<\/p>/gi;
  const matches = Array.from(contentHtml.matchAll(rowPattern));
  ...
  while (i < matches.length) {
    ...
    output += contentHtml.slice(cursor, start);

    const block: RegExpMatchArray[] = [current];
    let nextCursor = start + current[0].length;
    let j = i + 1;
    while (j < matches.length) {
      const next = matches[j]!;
      const gap = contentHtml.slice(nextCursor, next.index ?? nextCursor).trim();
      if (gap) break;
      block.push(next);
      nextCursor = (next.index ?? nextCursor) + next[0].length;
      j += 1;
    }

    const lines = block.map((item) => stripSimpleTag(item[0])).filter((line) => /^\|.+\|$/.test(line));
    let rendered: string | null = null;
    if (lines.length >= 3 && isMarkdownTableSeparator(lines[1]!)) {
      rendered = renderMarkdownTable(lines);
    }
    ...
    if (rendered) {
      ...
      output += current[0];
      cursor = start + current[0].length;
      i += 1;
    }
  }

  output += contentHtml.slice(cursor);
  return output;
}

function renderSimpleMarkdown(body: string): string {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('>')) {
      ...
      while (i < lines.length) {
        ...
        quoteLines.push(renderInlineMarkdown(candidate.replace(/^>\s?/, '')));
        i += 1;
      }
    ...
    if (/^[-*]\s+/.test(line)) {
      ...
      while (i < lines.length) {
        ...
        items.push(`<li>${renderInlineMarkdown(candidate.replace(/^[-*]\s+/, ''))}</li>`);
        i += 1;
      }
    ...
    if (/^\d+\.\s+/.test(line)) {
      ...
      while (i < lines.length) {
        ...
        items.push(`<li>${renderInlineMarkdown(candidate.replace(/^\d+\.\s+/, ''))}</li>`);
        i += 1;
      }
    ...
    if (/^\|.+\|$/.test(line) && i + 1 < lines.length && isMarkdownTableSeparator((lines[i + 1] ?? '').trim())) {
      ...
      const rendered = renderMarkdownTable(tableLines);
      if (rendered) {
...
async function parseMarkdownWithContext(markdownPath: string, options?: ParseOptions): Promise<ParsedResult> {
  ...
  if (options?.style) generateArgs.push('--style', options.style);
  ...
  if (result.status === 0 && fs.existsSync(tempHtmlPath)) {
    const generatedHtml = fs.readFileSync(tempHtmlPath, 'utf-8');
    contentHtml = extractContentArea(generatedHtml);
  } else {
    const stderr = result.stderr?.toString().trim() || 'unknown error';
    console.error(`[md-to-wechat-context] context-to-html unavailable, fallback to local renderer: ${stderr}`);
    contentHtml = renderSimpleMarkdown(modifiedBody);
  }
  ...
  contentHtml = transformMarkdownTables(contentHtml);
...
async function main(): Promise<void> {
  ...
  const result = await parseMarkdownWithContext(markdownPath, { title, style, htmlOutPath, tempDir });
