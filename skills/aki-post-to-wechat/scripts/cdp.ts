...
export function getDefaultProfileDir(profileName?: string): string {
...
export function resolveProfileDir(profileDir?: string, profileName?: string): string {
...
function findExistingChromeDebugPort(profileDir: string): number | null {
  ...
  // Use `auxww` to avoid argv truncation on macOS, otherwise
  // `--user-data-dir`/`--remote-debugging-port` can be cut off.
...
export async function launchChrome(
  ...
  const profile = resolveProfileDir(profileDir, profileName);
  ...
  // Do not keep Node process alive solely because Chrome child exists.
  chrome.unref();
