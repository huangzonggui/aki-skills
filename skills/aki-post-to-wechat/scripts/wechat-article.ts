...
async function parseMarkdownWithPlaceholders(markdownPath: string): Promise<{ title: string; author: string; summary: string; htmlPath: string; contentImages: ImageInfo[] }> {
  const scriptDir = path.dirname(new URL(import.meta.url).pathname);
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  const { title, content, htmlFile, markdownFile, author, summary, images = [], submit = false, profileDir, skipTitle = false, manualPaste = false } = options;
  ...
  if (markdownFile) {
    ...
    const parsed = await parseMarkdownWithPlaceholders(markdownFile);
...
async function main(): Promise<void> {
  ...
  await postArticle({ title: title || '', content, htmlFile, markdownFile, author, summary, images, submit, profileDir, skipTitle, manualPaste });
}
