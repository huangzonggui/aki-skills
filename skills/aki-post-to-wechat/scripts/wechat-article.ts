...
async function isArticleEditorReady(session: ChromeSession): Promise<boolean> {
  return await evaluate<boolean>(session, `
    (function() {
      return !!document.querySelector('#title')
        && !!document.querySelector('.ProseMirror')
        && !!document.querySelector('#js_submit button');
    })()
  `);
}

async function openArticleEditorDirect(session: ChromeSession, token: string): Promise<boolean> {
  const candidateUrls = [
    `https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit_v2&action=edit&isNew=1&type=77&lang=zh_CN&token=${token}`,
    `https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit&action=edit&isNew=1&type=77&lang=zh_CN&token=${token}`,
  ...
  for (const url of candidateUrls) {
    try {
      console.log(`[wechat] Direct article navigate attempt: ${url}`);
      await session.cdp.send('Page.navigate', { url }, { sessionId: session.sessionId });
      await sleep(2500);
      if (await isArticleEditorReady(session)) {
        console.log('[wechat] Direct article navigation succeeded.');
        return true;
      }
    } catch (err) {
      console.warn(`[wechat] Direct article navigate failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  return false;
}
...
async function parseMarkdownWithPlaceholders(markdownPath: string, style?: string): Promise<{ title: string; author: string; summary: string; htmlPath: string; contentImages: ImageInfo[] }> {
  const scriptDir = path.dirname(new URL(import.meta.url).pathname);
  const mdToWechatScript = path.join(scriptDir, 'md-to-wechat-context.ts');
  const bunBin = process.env.BUN_BIN || process.env.BUN_PATH || 'bun';
  const parsed = path.parse(markdownPath);
  const htmlOutPath = path.join(parsed.dir, `${parsed.name}.wechat.html`);
  const args = [mdToWechatScript, markdownPath, '--html-out', htmlOutPath];
  ...
  console.log(`[wechat] Generated HTML: ${parsedResult.htmlPath}`);
  return parsedResult;
}
...
async function waitForSelector(session: ChromeSession, selector: string, timeoutMs = 12_000): Promise<void> {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const exists = await evaluate<boolean>(session, `!!document.querySelector(${JSON.stringify(selector)})`);
    if (exists) return;
    await sleep(200);
  }
...
async function fillField(session: ChromeSession, selector: string, value: string, label: string): Promise<void> {
  await waitForSelector(session, selector);

  const jsSet = await evaluate<{ found: boolean; value: string }>(session, `
    (function() {
  ...
  if (!jsSet.found) {
    throw new Error(`${label} field not found: ${selector}`);
  }

  if (jsSet.value === value) {
    console.log(`[wechat] ${label} set (${jsSet.value.length} chars).`);
    return;
  }

  console.warn(`[wechat] ${label} JS set mismatch, falling back to keyboard input...`);
  ...
  const finalValue = await evaluate<string>(session, `(document.querySelector(${JSON.stringify(selector)})?.value || '')`);
  if (finalValue !== value) {
    throw new Error(`${label} fill failed. expected=${value.length}, actual=${finalValue.length}`);
  }
  console.log(`[wechat] ${label} set (${finalValue.length} chars).`);
}
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  if (markdownFile) {
    ...
    const parsed = await parseMarkdownWithPlaceholders(markdownFile, style);
  ...
  const { cdp } = await launchChrome(WECHAT_URL, profileDir, profileName);
  ...
  try {
    ...
    await sleep(1000);
    let homeToken = extractTokenFromUrl(await evaluate<string>(session, 'window.location.href'));
    try {
      await ensureHome(session);
      homeToken = extractTokenFromUrl(await evaluate<string>(session, 'window.location.href')) || homeToken;
    } catch (err) {
      console.warn(`[wechat] Home page fallback: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
      ...
      try {
        ...
        console.warn('[wechat] New tab not detected, trying editor fallback...');
        await sleep(1500);
        const targetsNow = await cdp.send<{ targetInfos: Array<{ targetId: string; url: string; type: string }> }>('Target.getTargets');
        const editorTarget = targetsNow.targetInfos.find((t) =>
          t.type === 'page'
          && t.url.includes('mp.weixin.qq.com')
          && (
            t.url.includes('appmsg')
            || t.url.includes('operate_appmsg')
            || t.url.includes('media/appmsg_edit')
          )
        );

        if (editorTarget) {
          editorTargetId = editorTarget.targetId;
          console.log('[wechat] Using existing editor tab.');
        } else {
          throw new Error('Editor tab not found after clicking "文章".');
        }
      }
    } catch (err) {
      if (!homeToken) {
      ...
      console.warn(`[wechat] Menu path unavailable, trying direct editor URL: ${err instanceof Error ? err.message : String(err)}`);
      const ok = await openArticleEditorDirect(session, homeToken);
      if (!ok) {
        throw new Error('Editor page not ready via menu path or direct navigation.');
      }
      editorTargetId = session.targetId;
    }
    ...
    if (effectiveTitle && !skipTitle) {
      ...
      await fillField(session, '#title', effectiveTitle, 'Title');
    }

    if (effectiveAuthor) {
      console.log('[wechat] Filling author...');
      await fillField(session, '#author', effectiveAuthor, 'Author');
    }
    ...
    if (effectiveHtmlFile && fs.existsSync(effectiveHtmlFile)) {
      ...
      if (manualPaste) {
        ...
        if (contentLength < 10) {
          ...
          if (retryLength < 10) {
            console.warn('[wechat] CDP paste still empty, falling back to direct HTML insertion...');
            const inserted = await insertHtmlDirect(session, effectiveHtmlFile);
            await sleep(1500);
            const directLength = await evaluate<number>(session, 'document.querySelector(".ProseMirror")?.textContent?.trim().length || 0');
            if (!inserted || directLength < 10) {
              throw new Error('Paste failed: editor still empty after retry and direct HTML insertion.');
            }
          }
        }
      }
...
Options:
  ...
  --markdown <path>  Markdown file to convert with aki-context-to-html and post
  --style <name>     Style preset for markdown mode (for example: part-guide)
...
Examples:
  ...
  npx -y bun wechat-article.ts --markdown article.md --submit
  npx -y bun wechat-article.ts --markdown article.md --style part-guide --submit
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--profile-name' && args[i + 1]) profileName = args[++i];
  }
